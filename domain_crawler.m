function [U,G] = domain_crawler(root,n)
...
while j < n & get(quit,'value') == 0
   ...
   for f = findstr('https://',page);

      % A link starts with 'https://' and ends with the next quote.
      min_domain=findstr('/',page(f+8:end));
      e = min([findstr('"',page(f:end)) findstr('''',page(f:end)) min_domain+8]);
